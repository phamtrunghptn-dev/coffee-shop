    ...
    if (cartItems && productContainer) {
        ...
        Object.values(cartItems).map(item => {
            ...
            <div class="product-box" > 
                <div class="product">
                    <ion-icon onclick="removeItemInCart()" class="removeItem" name="close-circle-outline"></ion-icon>
...
for( let j=0;j< removeItem.length; j++){
    removeItem[j].addEventListener('click', () =>{
        removeItemInCart(products[i]);
    })
}

    function removeItemInCart(product){
        let cartItems = localStorage.getItem('productInCart');
        cartItems = JSON.parse(cartItems);
